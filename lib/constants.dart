import 'package:flutter/material.dart';

const bcolor = Color(0xff133c55);
const bcolor1 = Color(0xffD8D8D8);
const bcolor2 = Color(0xff59a5d8);
const bcolor3 = Color.fromRGBO(41, 91, 133, 1);
const bcolor4 = Color(0xff4585b9);
const bcolor5 = Color(0xff11cdf3); //icons
const uColor = Color.fromARGB(255, 167, 160, 160);
const fColor = Color(0xff295b85);
const fColor2 = Color.fromARGB(255, 57, 63, 68);
const backgroundColor = Color(0xff0a151e);
const dColor = Color(0xff690d0d);
const dPrimaryColor = Color(0xff133c55);
const lPrimaryColor = Color(0xff376fa4);
